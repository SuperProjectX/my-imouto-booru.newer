...
class Rails_Xml
{
    ...
    {
        if ($el instanceof ActiveRecord_Base) {
            $attrs = $el->to_xml();
        } elseif (is_array($el)) {
            $attrs = $el;
        } else {
            Rails::raise('InvalidArgumentException',
                "%s accepts either a child of ActiveRecord_Base or an array, %s passed",
                [__METHOD__, gettype($attrs)]);
        }
        
        if (!isset($params['root']))
            Rails::raise('InvalidArgumentException', "Missing 'root' parameter for %s", __METHOD__);
    ...
    public function create()
    {
        ...
        foreach ($this->_attrs as $name => $val) {
            ...
            $attrs_str[] = $name . '="'.htmlentities((string)$val).'"';
        }
