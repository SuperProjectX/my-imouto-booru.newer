...
abstract class Post_ImageStore_Base
{
    ...
    {
        $file = dirname(__FILE__) . '/' . CONFIG()->image_store . '.php';
        
        if (!is_file($file))
            Rails::raise("Exception", "File not found for image store configuration '%s'.", CONFIG()->image_store ?: '[empty value]');
        
        require_once $file;
        
        $class = 'Post_ImageStore_' . Rails::lower_to_camel(CONFIG()->image_store);
    ...
    {
        $dirs = array_filter(explode('/', str_replace(RAILS_ROOT, '', pathinfo($dir, PATHINFO_DIRNAME))));
        $dir = RAILS_ROOT . '/';
        foreach ($dirs as $d) {
            $dir .= $d . '/';
