...
trait PostSqlMethods
{
    static public function find_by_tag_join($tag, array $options = array())
    {
        ...
        $posts = self::find_all(array('conditions' => array("tags.name = ?", $tag), 'select' => "posts.*", 'joins' => "JOIN posts_tags ON posts_tags.post_id = posts.id JOIN tags ON tags.id = posts_tags.tag_id", 'limit' => $options['limit'], 'offset' => $options['offset'], 'order' => $options['order']));
    ...
    static public function generate_sql_range_helper($arr, $field, &$c, &$params)
    {
        switch ($arr[0]) {
            case 'eq':
                $c[] = $field." = ?";
