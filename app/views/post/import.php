<?php if ($this->request()->post()) : ?>
  ...
  <div id="script<?= $this->params()->post['i'] ?>" style="display:none">
    Import.set_status(<?= $this->params()->post['i'] ?>, '<?= $this->status ?>');
    Import.tags.push('<?=  $this->post->tags ?>');
    <?php if ($this->status != 'Posted') : ?>Import.error_count();<?php endif ?>
...
<h2><?= $this->link_to('Import', '#import', ['style' => 'color:white;']) ?></h2>
<br />
    ...
    <table class="form">
      ...
      <tbody>
        ...
        <tr>
          <th><label for="post_rating_questionable">Rating</label></th>
          <td>
            <?= $this->radio_button_tag('post[rating]', 'e', CONFIG()->default_rating_import == 'e', ['tabindex' => 4, 'id' => 'post_rating_explicit']) ?>
            <label for="post_rating_explicit">Explicit</label>

            <?= $this->radio_button_tag('post[rating]', 'q', CONFIG()->default_rating_import == 'q', ['tabindex' => 4, 'id' => 'post_rating_questionable']) ?>
            <label for="post_rating_questionable">Questionable</label>

            <?= $this->radio_button_tag('post[rating]', 's', CONFIG()->default_rating_import == 's', ['tabindex' => 4, 'id' => 'post_rating_safe']) ?>
