...
<div class="content">
  <?= $this->form_tag(['action' => "diff"], ['method' => 'get'], function(){ ?>
    <?= $this->hidden_field_tag("title", $this->params()->title) ?>
    ...
    <table id="history" width="100%">
      ...
      <tfoot>
        <tr>
          <td colspan="2"><?= $this->submit_tag("Compare") ?></td>
        </tr>
      </tfoot>
      <tbody>
        <?php foreach ($this->wiki_pages as $i => $wiki_page) : ?>
          <tr class="<?= $this->cycle('even', 'odd') ?>">
            <td><?= $this->radio_button_tag("from", $wiki_page->version, $i == 1, ['id' => "from_".$wiki_page->version]) ?></td>
            <td><?= $this->radio_button_tag("to", $wiki_page->version, $i == 0, ['id' => "to_".$wiki_page->version]) ?></td>
            <td><?= $this->link_to(date("Y-m-d H:i", strtotime($wiki_page->updated_at)), ['action' => "show", 'title' => $wiki_page->title, 'version' => $wiki_page->version]) ?></td>
            <td><?= $this->link_to($wiki_page->author(), ['controller' => 'user', 'action' => 'show', 'id' => $wiki_page->user_id]) ?></td>
