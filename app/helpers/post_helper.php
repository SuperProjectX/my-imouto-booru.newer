<?php
class PostHelper extends ActionView_Helper
{
    public function print_preview($post, $options = array())
    {
        ...
        $image_title = $is_post ? $this->h("Rating: ".$post->pretty_rating()." Score: ".$post->score." Tags: ".$this->h($post->tags." User: ".$post->author)) : null;
    ...
    public function auto_discovery_link_tag_with_id($type = 'rss', $url_options = array(), $tag_options = array())
    {
        if (is_array($url_options)) {
            ...
            $href = $this->url_for($url, $url_options);
        } else {
            $href = $url_options;
        }
        return $this->tag(
